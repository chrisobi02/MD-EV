        ...
        for i, instance in data[instance_type].iterrows():
            ...
            try:
                ...
                job_start_locations = [
                    ...
                    for start in instance["tripStartLocations"].split(";")
                ]
                job_end_locations = [
                    ast.literal_eval(end)
                    for end in instance["tripEndLocations"].split(";")
                ]
                depot_location = [
                    ast.literal_eval(depot)
                    for depot in instance["depotLocations"].split(";")
                ]
                station_location = [
                    ast.literal_eval(station)
                    for station in instance["stationLocations"].split(";")
                ]
            except:
                print(f"{instance['label']} failed due to truncated data")

            if (
                not len(job_start_time)
                == len(job_end_time)
                == len(job_charge)
                == len(job_start_locations)
                == len(job_end_locations)
            ):
            ...
            buildings = (
                []
            )  # Abstract buildings into a list so coordiantes aren't stored on the job level
            depots = []
            ...
            for i in range(len(job_start_time)):
                ...
                job_start_entrance, job_start_location = (
                    job_start_locations[i][:2],
                    job_start_locations[i][2:],
                )
                job_end_entrance, job_end_location = (
                    job_end_locations[i][:2],
                    job_end_locations[i][2:],
                )
                ...
                jobs.append(
                    {
                        ...
                        "start_location": tuple(job_start_location),
                        "end_location": tuple(job_end_location),
                    }
                )
            building_json = []
            ...
            for i, building in enumerate(buildings):
                ...
                if building in depots_just_entrance:
                    ...
                    building_kwargs["capacity"] = maximum_per_depot[
                        depots_just_entrance.index(building)
                    ]
                    building_kwargs["location"] = tuple(
                        depots[depots_just_entrance.index(building)][2:]
                    )
