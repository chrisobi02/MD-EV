...
class InterpolationIP(BaseMDEVCalculator):
    ...
    def write_solution(self, routes: list[list[Location]], suffix="") -> None:
        ...
        with open(f"{solution_directory}/c-{self.config.UNDISCRETISED_MAX_CHARGE}-{self.data['label']}{suffix}.json", "w") as f:
            json.dump(
                {
    ...
    def run(self, suffix=""):
        ...
        self.write_solution(routes, suffix=suffix)
        detoured = [self.add_detours_to_route(route) for route in routes]
        return detoured


class NoChargeIP(BaseMDEVCalculator):
    TYPE = "interpolation"
    ...
    def write_solution(self, routes: list[list[Location]], suffix="") -> None:
        solution_directory = f"{self.base_dir}/solutions/no-charge"
        # if output_override is not None:
        #     solution_directory = output_override
        if not os.path.exists(solution_directory):
            # If the directory doesn't exist, create it
